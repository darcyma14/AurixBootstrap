...
void EcuM_AL_DriverInitOne( const EcuM_ConfigType *configptr )
{
 ...
 #ifdef ECUM_USES_ADC
//del	IrqAdc_Init();    /* Initialise the interrupt priority for ADC */
 #endif
...
//DEL	IrqStm_Init();    /* Assign priorities for STM interrupts */
...
#ifdef ECUM_USES_GTM
//DEL	IrqGtm_Init();     /* Assign priorities for GTM interrupts */
#endif
