...
#define ASCLIN_BITCON_PRESCALAR (24U) /* 1 - 4096 */
...
#define ASCLIN_BRG_NUMERATOR       (288U)  /* 115200 Hz */
...
#define ASCLIN_CSR_CLKSEL       (ASCLIN_CSR_CLKSEL_CLC)//ASCLIN_CSR_CLKSEL_EVROSC)
...
/*~-*/
{
    register signed int xTimeOutCount = ( 1024 );
    /*~T*/
    /* send character */
    psASCLIN0->FLAGSCLEAR.U = ASCLIN_FLAGSCLEAR_MASK;
    ...
    /*~E*/
    /*~L*/
    while( ( psASCLIN0->TXFIFOCON.B.FILL ) != 0 && ( xTimeOutCount > 0L ) )
    /*~-*/
    {
        xTimeOutCount--;
        /*~T*/
