...
IFX_LOCAL_INLINE void Gtm_lEnableGtm(void);

#if (GTM_SFR_RESET_ENABLE == STD_ON)

IFX_LOCAL_INLINE void Gtm_lResetKernel(void);
...
void Gtm_Init(const Gtm_ConfigType *ConfigPtr)
{
  ...
  GTM_CTRL.U = (unsigned_int)(GeneralConfigPtr->GtmCtrlValue);
  GTM_IRQ_EN.U = (unsigned_int)((uint32)GeneralConfigPtr->GtmIrqEnable & \
                                                             GTM_GET_IRQ_VAL_8);

  GTM_IRQ_MODE.U = (unsigned_int)(((uint32)(GeneralConfigPtr->GtmIrqEnable) & \
                                     GTM_GET_MODE_VAL_8) >> GTM_SHIFT_TO_LSB_8);

  GTM_EIRQ_EN.U = (unsigned_int)(((uint32)(GeneralConfigPtr->GtmIrqEnable) & \
                                GTM_GET_EIRQ_VAL_8) >> GTM_SHIFT_EIRQ_TO_LSB_8);


  /* SRN Enable */
  SRC_GTMAEIIRQ.U |= (unsigned_int)(GTM_BIT_SET << GTM_BIT_SRE);
...
void Gtm_DeInit(void)
{
  ...
  GTM_KRST0.B.RST = 1U;
  ReadBack = GTM_KRST0.U;

  GTM_KRST1.B.RST = 1U;
  ReadBack = GTM_KRST1.U;

  /* Clear the kernel reset register */
  GTM_KRSTCLR.B.CLR = 1U;

  /* Read back the KRSTCLR register */
  ReadBack = GTM_KRSTCLR.U;

  /* Disable GTM Module */
  MODULE_GTM.CLC.B.DISR = GTM_ONE_U;
  ...
  for(MinorCnt = GTM_ZERO_U; MinorCnt < GTM_SIXTEEN_U; MinorCnt++)
  {
    ...
    (GTM_TOM_CHAN_ENABLE << (MinorCnt * GTM_TOM_BITS_PER_CHAN))) > GTM_ZERO_UL)
    {
      TomTgcRegPtr->ENDIS_CTRL.U =(unsigned_int)\
                (~(GTM_TOM_CHAN_DISABLE << (MinorCnt * GTM_TOM_BITS_PER_CHAN)));
  ...
  for(MajorCnt = GTM_ZERO_U; MajorCnt < GTM_NO_OF_TOM_MODULES; MajorCnt++)
  {
    for(MinorCnt = GTM_ZERO_U; MinorCnt < GTM_NO_OF_TGC_PER_MODULE;MinorCnt++)
    {
      ...
      if((TomTgcRegPtr->FUPD_CTRL.U & GTM_TOM_TGC_GET_FUPD) > GTM_ZERO_UL)
      {
        ...
        TomTgcRegPtr->GLB_CTRL.U |= (unsigned_int)GTM_BIT_SET;
      }
    }
  }
}
    ...
    for(MinorCnt = GTM_ZERO_U; MinorCnt < GTM_BITS_IN_U8 ; MinorCnt++)
    {
     ...
     PBConfigStructure and is within allowed range.*/
        ...
        if(TomConfigPtr->GtmTomTgcConfigGrpPtr != NULL_PTR)
        {
          ...
          TomTgcRegPtr->OUTEN_STAT.U = (unsigned_int)\
     ...
     TbuConfigPtr=&(Gtm_kConfigPtr->GtmModuleConfigPtr->GtmTbuConfigPtr[Count]);
      ...
      if(Gtm_lGetUnused8(TbuConfigPtr->TbuChannelCtrl) == (boolean)FALSE)
      {
        ...
        GtmTbu->GtmTbuCh[Count].CH_CTRL.U =                       \
                     (unsigned_int)((uint32)TbuConfigPtr->TbuChannelCtrl &     \
                     GTM_TBU_GET_LOWER_NIBBLE);
        GtmTbu->GtmTbuCh[Count].CH_BASE.U = TbuConfigPtr->TbuBaseValue;
        GTM_TBU_CHEN.U = ((unsigned_int)\
             (Gtm_lGetBit8(TbuConfigPtr->TbuChannelCtrl,GTM_TBU_CONFIGURED))   \
...
/*******************************************************************************
** Syntax : IFX_LOCAL_INLINE void Gtm_lResetKernel(void)                      **
**                                                                            **
** Service ID:       none                                                     **
**                                                                            **
** Sync/Async:       Synchronous                                              **
...
IFX_LOCAL_INLINE void Gtm_lResetKernel(void)
{
  ...
  GTM_KRST0.B.RST = 1U;
  ResetReadBack = GTM_KRST0.U;
  GTM_KRST1.B.RST = 1U;
  ResetReadBack = GTM_KRST1.U;
  ...
  {
    #ifdef IFX_GTM_DEBUG
    ResetStatus = (GTM_KRST0.B.RSTSTAT & TestGtm_DebugMask01);
    #else
    ResetStatus = (uint32)(GTM_KRST0.B.RSTSTAT);
    #endif
