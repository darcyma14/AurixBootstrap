       ...
       (GTM_BIT_SET << MinorCnt)) > GTM_ZERO_UL)
    {
     ...
     PBConfigStructure and is within allowed range.*/
      ...
      if(TimConfigPtr->TimUsage == GTM_DRIVER_COMPLEX)
      {
        ...
        TimChannelRegPtr->CH_CTRL.U =(unsigned_int)\
       ...
       /* Update the TIM External Control register if EXT_CAP_EN is enabled */
        TimChannelRegPtr->CH_ECTRL.U |= (unsigned_int)                      \
                 (TimConfigPtr->TimExtCapSrc & GTM_TIM_EXT_CAP_ENABLE_MASK);


        /* Enable SRN */
        MODULE_SRC.GTM.GTM[0].TIM[0U][ChannelNo].U |=        \
...
void Gtm_lResetGtmSRCReg(void)
{
  ...
  SRC_GTMAEIIRQ.U |= GTM_CLEAR_STICKY_BITS;

  /* Disable SRE  */
  SRC_GTMAEIIRQ.B.SRE = GTM_ZERO_U;
  ...
  {
    /* Clear Sticky bits of SRC register */
    MODULE_SRC.GTM.GTM[0U].TIM[0U][ChannelNo].U |= GTM_CLEAR_STICKY_BITS;
    /* Disable SRE  */
    MODULE_SRC.GTM.GTM[0U].TIM[0U][ChannelNo].B.SRE = GTM_ZERO_U;
  }
  ...
  for(ModuleNo=GTM_ZERO_U; ModuleNo < GTM_NO_OF_TOM_MODULES; ModuleNo++)
  {
                                                                 ...
                                                                 ChannelNo++)
    {
      ...
      /* Disable SRE  */
      MODULE_SRC.GTM.GTM[0U].TOM[ModuleNo][ChannelNo].B.SRE = GTM_ZERO_U;
    }
  }
}
       ...
       if(TomChannelConfigPtr->TomUsage != GTM_DRIVER_COMPLEX)
        {
         ...
         Permitted for special function registers.*/
          ...
          /*Set the CTRL value*/
          TomChannelRegPtr->CTRL.B.CLK_SRC_SR  = (uint8)                      \
