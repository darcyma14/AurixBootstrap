...
IFX_LOCAL_INLINE void Icu_lDeInitEru
(
  Icu_17_GtmCcu6_ChannelType EruEicrUsed,
  Icu_17_GtmCcu6_ChannelType EruEicrChannel
);
...
IFX_LOCAL_INLINE void Icu_lGtmStartEdgeDetect(
...
IFX_LOCAL_INLINE void Icu_lCcu6T12DeInit(void);
#endif
/* #if (ICU_DE_INIT_API == STD_ON) */

#if (ICU_CCU6_USED == STD_ON)

IFX_LOCAL_INLINE void Icu_lCcu6T12Initialization(void);
...
/* #if (ICU_CCU6_USED == STD_ON) */
    ...
    MODULE_SRC.SCU.SCU.ERU[0].U |= 
      (unsigned_int)(ICU_SET_BIT << ICU_SRC_SRE_POS);
   /* _Ifx_SRC_GSCU */
    #endif
...
/* #if (ICU_ERU_USED == STD_ON) */
      ...
      if(ChlType == ICU_CCU6_ASIL_CHANNEL)
      {
       ...
       if(ChlType == ICU_CCU6_QM_CHANNEL)
       {
        ...
        Ccu6RegPtr->T12MSEL.U &= (unsigned_int)~(ICU_CCU_MSEL_RESET_VAL << \
                             (CcChannelNumber * ICU_CCU_MSEL_BITS_PER_CHANNEL));

        /* Port connections initialized to its reset value */
        Ccu6RegPtr->PISEL0.U &= (unsigned_int)~(ICU_CCU_PISEL_RESET_VAL << \
                            (CcChannelNumber * ICU_CCU_PISEL_BITS_PER_CHANNEL));

        /* Clear the IEN register */
        Ccu6RegPtr->IEN.U &= (unsigned_int)~(ICU_CCU_CLEAR_INTERRUPT << \
        ...
        Ccu6RegPtr->ISR.U |= (unsigned_int)(ICU_CCU_CLEAR_INTERRUPT << \
                               (CcChannelNumber * ICU_CCU_IS_BITS_PER_CHANNEL));
...
#if (ICU_ERU_USED == STD_ON)
/******************************************************************************
** Syntax :           IFX_LOCAL_INLINE void Icu_lDeInitEru                    **
       (Icu_17_GtmCcu6_ChannelType EruEicrUsed,
                         Icu_17_GtmCcu6_ChannelType EruEicrChannel)          **
**                                                                           **
** Service ID:        none                                                   **
...
**                                                                           **
******************************************************************************/
IFX_LOCAL_INLINE void Icu_lDeInitEru(Icu_17_GtmCcu6_ChannelType EruEicrUsed, 
                                    Icu_17_GtmCcu6_ChannelType EruEicrChannel)
{                                                            
    ...
    (unsigned_int)(((uint32)(LocalIcuChanPtr->IcuProperties.
      ...
      TimChannelRegPtr->CH_FLT_RE.U = 
       (unsigned_int)(LocalIcuChanPtr->IcuProperties.
                        TimChFilterTimeForRisingEdge);
      TimChannelRegPtr->CH_FLT_FE.U = 
       (unsigned_int)(LocalIcuChanPtr->IcuProperties.
                      TimChFilterTimeForFallingEdge);
                                ...
                                uint8 ActiveEdge)
{
  ...
  TimChannelRegPtr->CH_CTRL.U &= 
                         (unsigned_int)~(ICU_THREE_UL << ICU_TIM_CTRL_DSL_POS);
  TimChannelRegPtr->CH_CTRL.U |=(unsigned_int)((ICU_GTM_TIM_MODE_TIEM << 1) |  \
...
static void Icu_lGtmResetChannels(void)
{
  ...
  PBConfigStructure and is within allowed range.*/ 
    ...
    if(LocalIcuChanPtr->IcuProperties.AssignedHwUnit == ICU_GTM_OPTION)        
    {
      ...
      TimInScrRegPtr->RST.U = (unsigned_int)(ICU_SET_BIT << (ChannelNumber)); 
    }
  }
}
#endif
                                   ...
                                   )
{
  ...
  TimChannelRegPtr->CH_CTRL.U &= (unsigned_int)(~ICU_GTM_TIM_ENABLE_CHANNEL);
} 
#endif
...
/*IFX_MISRA_RULE_11_05_STATUS=volatile in terms of pointer access*/
  TimChannelRegPtr = &(((*(Ifx_GTM_TIMx*)(void *)(MODULE_GTM.TIM)).
                                               CH_TIM[ModuleNo].CH[ChannelNo]));
  Cictrlvalue = (uint32)(((unsigned_int)(TimChannelRegPtr->CH_CTRL.U)) & \
  ...
  TimChannelRegPtr->CH_CTRL.U |= (unsigned_int)Cictrlvalue;
  /* Channel is enabled */
  TimChannelRegPtr->CH_CTRL.U |= ICU_GTM_TIM_ENABLE_CHANNEL ;  
  /* Clear all pending interrupts */
  TimChannelRegPtr->CH_IRQ_NOTIFY.U = ICU_GTM_CLEAR_INTERRUPTS;
  ...
  if((SignalMode & ICU_TIM_GET_LOWER_NIBBLE_U) == 0U)
  {
    ...
    TimChannelRegPtr->CH_IRQ_EN.U  |= (unsigned_int)ICU_TIM_IRQ_ECNT_MASK;      
  }
}
                        ...
                        uint8 ChlType)
{
  ...
  {
    Newvalue = (uint8)(((uint32)(TimChannelRegPtr->CH_IRQ_NOTIFY.U)) & \
    ...
    if((Newvalue == 1U)||(*InfoPtr == ICU_TWO_U))
    {
      *InfoPtr = 0U;
      ...
      RetVal = 
      (uint32)(TimChannelRegPtr->CH_GPR1.U & (unsigned_int)GTM_TIMER_MASK);
    }
    else
    {
      RetVal = 0UL;
    ...
    if(Newvalue == 1U)
    {
      ...
      TimChannelRegPtr->CH_IRQ_NOTIFY.U |=
                                  (unsigned_int)(1UL <<ICU_GTM_TIM_NEWVAL_IRQ);
      *InfoPtr = 1U;
    }
  }
  else
...
#if (ICU_CCU6_USED == STD_ON)
/*******************************************************************************
** Syntax : IFX_LOCAL_INLINE void Icu_lCcu6T12Initialization(void)            **
**                                                                            **
** Service ID:       none                                                     **
**                                                                            **
** Sync/Async:       Synchronous                                              **
...
IFX_LOCAL_INLINE void Icu_lCcu6T12Initialization(void)
{
      ...
      KernelCount++)
  {
    ...
    (ICU_17_GTMCCU6_RESET_SFR_AT_INIT == STD_ON)) && (ICU_CCU6_USED == STD_ON))
/*******************************************************************************
** Syntax : IFX_LOCAL_INLINE void Icu_lCcu6T12DeInit(void)                    **
**                                                                            **
** Service ID:       none                                                     **
**                                                                            **
** Sync/Async:       Synchronous                                              **
      ...
      KernalCount++)
  {
    ...
    {     
     /* Get the base address of the Kernal */
      ...
      /* Initialize the counter to 0 */
      Ccu6RegPtr->T12.U = 0UL;
      Ccu6RegPtr->TCTR0.U &= (unsigned_int)ICU_CCU_TCTR0_RESET_VAL;        
                        ...
                        const Icu_17_GtmCcu6_ChannelConfigType *LocalIcuChanPtr)
{
                                                 ...
                                                 == ICU_MODE_SIGNAL_EDGE_DETECT)
  {
    ...
    Ccu6RegPtr->T12MSEL.U |= (unsigned_int)(ICU_CCU_CAPTURE_MODE1 << \
                             (CcChannelNumber * ICU_CCU_MSEL_BITS_PER_CHANNEL));
    ...
    Ccu6RegPtr->IEN.U |= (unsigned_int)((uint32)CcuActiveEdge << \
                               (CcChannelNumber * ICU_CCU_IS_BITS_PER_CHANNEL));
                           ...
                           Icu_17_GtmCcu6_ActivationType CfgEdge)
{
  ...
  if(SigKind == ICU_PERIOD_TIME)
  {
    if(CfgEdge == ICU_RISING_EDGE)
    {
      ...
      Ccu6RegPtr->T12MSEL.U |= (unsigned_int)(ICU_CCU_CAPTURE_MODE2 << \
                           (CcChannelNumber * ICU_CCU_MSEL_BITS_PER_CHANNEL));
    }
    else
    {
        /* Channel is configured to Capture Mode 3 */
      Ccu6RegPtr->T12MSEL.U |= (unsigned_int)(ICU_CCU_CAPTURE_MODE3 << \
                             (CcChannelNumber * ICU_CCU_MSEL_BITS_PER_CHANNEL));
    }      
  }
  else
                                ...
                                uint8 ActiveEdge, uint32 EdgeCount)
{
  ...
  TimChannelRegPtr->CH_CTRL.U &= 
                         (unsigned_int)~(ICU_THREE_UL << ICU_TIM_CTRL_DSL_POS);
  TimChannelRegPtr->CH_CTRL.U |= 
                           (unsigned_int)((ICU_GTM_TIM_MODE_TIPM << 1)   |   \
                                (((uint32)ActiveEdge) << ICU_TIM_CTRL_DSL_POS));
  TimChannelRegPtr->CH_CNTS.U = 0UL;
  TimChannelRegPtr->CH_CNTS.U = (unsigned_int)(EdgeCount - 1UL);
  TimChannelRegPtr->CH_CTRL.U |= ICU_GTM_TIM_ENABLE_CHANNEL ; 
}
