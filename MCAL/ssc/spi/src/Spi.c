...
IFX_LOCAL_INLINE Spi_StatusType Spi_lGetHwStatus(void);
...
IFX_LOCAL_INLINE void Spi_lEnableChipSelect(uint16 CSPin, uint8 CSPolarity,
                                           Spi_HWUnitType  HwUnit);

/* Function to Disable Chip Select*/
IFX_LOCAL_INLINE void Spi_lDisableChipSelect(uint16 CSPin, uint8 CSPolarity,
                                             Spi_HWUnitType  HwUnit);
...
IFX_LOCAL_INLINE void Spi_lSyncTransmitConfigPortPin
(
...
IFX_LOCAL_INLINE uint32 Spi_lHwCheckRxStatus(volatile Ifx_QSPI* ModulePtr);
...
IFX_LOCAL_INLINE void Spi_lHwUpdateSourceReg(uint32 Module, uint16 DataWidth,
                                   ...
                                   || (SPI_LEVEL_DELIVERED == 2U))
IFX_LOCAL_INLINE Dma_ErrorStatusType Spi_lDmaGetBothMoveEngErrFlags(void);
#endif
...
IFX_LOCAL_INLINE void Spi_lHwEnableDmaIntr(Dma_ChannelType Channel);
IFX_LOCAL_INLINE void Spi_lHwDisableDmaIntr(Dma_ChannelType Channel);
...
IFX_LOCAL_INLINE void Spi_lHwDisableQspiTxIntr(uint32 Module);
IFX_LOCAL_INLINE void Spi_lHwEnableQspiTxIntr(uint32 Module);
IFX_LOCAL_INLINE void Spi_lHwEnableQspiRxIntr(uint32 Module);
...
#if (SPI_LEVEL_DELIVERED == 2U)
IFX_LOCAL_INLINE void Spi_lHwUpdateErrIntr(uint32 Module, uint32 Value);
#endif

IFX_INLINE void Spi_lHwDisableModule(uint32 Module);
...
IFX_INLINE void Spi_lHwClrQspiPendEvent(Ifx_QSPI *ModulePtr);
...
IFX_INLINE void Spi_lHwGpioSetPortPin(uint16 CSPin, uint8 CSPolarity);
...
Std_ReturnType Spi_InitCheck(const Spi_ConfigType* ConfigPtr)
{
  ...
  #if ((SPI_QM_ASYNC_MASTER_MODULES_USED == 0U)&&(SPI_LEVEL_DELIVERED != 0U))
...
Std_ReturnType Spi_SyncTransmit(Spi_SequenceType Sequence)
{
  ...
  /* ( (SPI_DEV_ERROR_DETECT == STD_ON) || (SPI_SAFETY_ENABLE == STD_ON) ) */
  {
    ...
    /* The synchronous service is already started. No other
     ...
     mechanism*/
    Synclock = Mcal_LockResource(&Spi_SyncLock);
...
void Spi_IsrQspiUsr(uint32 Module)
{
  volatile Ifx_QSPI* ModulePtr;
   ...
   /* (SPI_SAFETY_ENABLE == STD_ON) */
   {
     ...
     /*IFX_MISRA_RULE_11_05_STATUS="SPI_HW_MODULE" attempt to cast away volatile
       in terms of pointer access.*/
...
** Syntax : IFX_LOCAL_INLINE void Spi_lEnableChipSelect                       **
...
/*******************************************************************************
** Syntax : IFX_LOCAL_INLINE void Spi_lDisableChipSelect(uint16 CSPin,        **
...
static void Spi_lStartJob(uint32 Module, Spi_JobType JobId)
{
  ...
  /*IFX_MISRA_RULE_11_05_STATUS="SPI_HW_MODULE" attempt to cast away volatile in
    terms of pointer access.*/
  /*IFX_MISRA_RULE_17_04_STATUS="SPI_HW_MODULE" Pointer arithmetic is used to
    efficiently access the SFRs of multiple QSPI Modules.*/
  ModulePtr =  &(SPI_HW_MODULE[Module]);
...
/*******************************************************************************
** Syntax : IFX_LOCAL_INLINE Spi_StatusType Spi_lGetHwStatus(void)            **
**                                                                            **
** Service ID:  None                                                          **
**                                                                            **
** Sync/Async:  Synchronous                                                   **
...
)
{
  ...
  else
  {
    do
    {
      /*IFX_MISRA_RULE_11_05_STATUS="DataBufferPtr" attempt to cast away
        volatile in terms of pointer access.*/
...
#if(SPI_LEVEL2_ASYNC_ONLY == STD_OFF)
/*******************************************************************************
** Syntax : IFX_LOCAL_INLINE void Spi_lSyncTransmitConfigPortPin              **
** (                                                                          **
**   Ifx_QSPI* ModulePtr,                                                     **
**   uint16 hwpin,                                                            **
**   uint8 Polarity                                                           **
...
*******************************************************************************/
IFX_LOCAL_INLINE void Spi_lSyncTransmitConfigPortPin(const Ifx_QSPI* ModulePtr,
                                    uint16 hwpin, uint8 Polarity )
{
  volatile uint32 cntr;
  cntr = SPI_LOOPCNTR_MAX;
...
static void Spi_lHwDeInit(void)
{
  ...
  do
  {
    if(Spi_lModuleConfigured((uint8)ModLoopCtr) != 0U)
    {
      ...
      {
        ModulePtr->ECON[HwChnlIdx].U = SPI_QSPI_ECON_RSTVAL;
      }
    }
    ModLoopCtr++;
  }while (ModLoopCtr < SPI_MAX_HW_UNIT);
...
#endif
/*******************************************************************************
** Syntax           : IFX_LOCAL_INLINE uint32 Spi_lHwCheckRxStatus            **
** (                                                                          **
**   Ifx_QSPI* ModulePtr,                                                     **
** )                                                                          **
**                                                                            **
...
/*IFX_MISRA_RULE_17_04_STATUS="SpiBaudrateEconPtr" Pointer arithmetic is used
      ...
      ReadValue = ModulePtr->ECON[HwChannelno].U;
...
IFX_LOCAL_INLINE void Spi_lQSPIHwKernelReset(uint32 Module)
{
  ...
  /*IFX_MISRA_RULE_17_04_STATUS="SPI_HW_MODULE" Pointer arithmetic is used to
    efficiently access the SFRs of multiple QSPI Modules.*/
  SPI_HW_MODULE[Module].KRST0.U = SPI_KERNEL_RESET_VALUE;
  /*IFX_MISRA_RULE_11_05_STATUS="SPI_HW_MODULE" attempt to cast away
    volatile in terms of pointer access.*/
  /*IFX_MISRA_RULE_17_04_STATUS="SPI_HW_MODULE" Pointer arithmetic is used
    to efficiently access the SFRs of multiple QSPI Modules.*/
  Readback = SPI_HW_MODULE[Module].KRST0.U;

  /*IFX_MISRA_RULE_11_05_STATUS="SPI_HW_MODULE" attempt to cast away volatile in
    terms of pointer access.*/
  /*IFX_MISRA_RULE_17_04_STATUS="SPI_HW_MODULE" Pointer arithmetic is used to
    efficiently access the SFRs of multiple QSPI Modules.*/
  SPI_HW_MODULE[Module].KRST1.U = SPI_KERNEL_RESET_VALUE;

  /*IFX_MISRA_RULE_11_05_STATUS="SPI_HW_MODULE" attempt to cast away
    volatile in terms of pointer access.*/
  /*IFX_MISRA_RULE_17_04_STATUS="SPI_HW_MODULE" Pointer arithmetic is used
    to efficiently access the SFRs of multiple QSPI Modules.*/
  Readback = SPI_HW_MODULE[Module].KRST1.U;
  ...
  do
  {
    ...
    #else
    /*IFX_MISRA_RULE_11_05_STATUS="SPI_HW_MODULE" attempt 
      to cast away volatile in terms of pointer access.*/
  ...
  /*IFX_MISRA_RULE_17_04_STATUS="SPI_HW_MODULE" Pointer arithmetic
    is used to efficiently access the SFRs of multiple QSPI Modules.*/
  SPI_HW_MODULE[Module].KRSTCLR.U = SPI_KERNEL_RESET_VALUE;

  /*IFX_MISRA_RULE_11_05_STATUS="SPI_HW_MODULE" attempt to cast away
    volatile in terms of pointer access.*/
  /*IFX_MISRA_RULE_17_04_STATUS="SPI_HW_MODULE" Pointer arithmetic is used
    to efficiently access the SFRs of multiple QSPI Modules.*/
  Readback = SPI_HW_MODULE[Module].KRSTCLR.U;
...
#if (SPI_LEVEL_DELIVERED == 2U)
/*******************************************************************************
** Syntax           : IFX_LOCAL_INLINE void Spi_lHwUpdateErrIntr              **
** (                                                                          **
**   uint32 Module,                                                           **
**   uint32 Value                                                             **
** )                                                                          **
...
/*******************************************************************************
** Syntax           : IFX_LOCAL_INLINE void Spi_lHwDisableQspiTxIntr          **
** (                                                                          **
**   uint32 Module                                                            **
** )                                                                          **
**                                                                            **
...
static void Spi_lHwDmaResetSfr(void)
{
  ...
  MODULE_DMA.ERRINTR.U = SPI_DMA_RESET_VALUE;
  ...
  do
  {
    if(Spi_lModuleConfiguredAndAsync((uint8)ModLoopCtr) != 0U)
    {
      ...
      MODULE_DMA.CH[DmaTransmitChannel].ADICR.U = SPI_DMA_RESET_VALUE;
      MODULE_DMA.CH[DmaTransmitChannel].CHCSR.U = SPI_DMA_RESET_VALUE;
      MODULE_DMA.TSR[DmaTransmitChannel].U = SPI_DMA_RESET_VALUE;
      MODULE_DMA.CH[DmaTransmitChannel].SADR.U = SPI_DMA_RESET_VALUE;
      MODULE_DMA.CH[DmaTransmitChannel].CHCFGR.U = SPI_DMA_RESET_VALUE;
      MODULE_DMA.CH[DmaTransmitChannel].DADR.U = SPI_DMA_RESET_VALUE;
      /* Reset Rx DMA channel specific SFRs */
      MODULE_DMA.CH[DmaReceiveChannel].ADICR.U = SPI_DMA_RESET_VALUE;
      MODULE_DMA.CH[DmaReceiveChannel].CHCSR.U = SPI_DMA_RESET_VALUE;
      MODULE_DMA.TSR[DmaReceiveChannel].U = SPI_DMA_RESET_VALUE;
      MODULE_DMA.CH[DmaReceiveChannel].SADR.U = SPI_DMA_RESET_VALUE;
      MODULE_DMA.CH[DmaReceiveChannel].CHCFGR.U = SPI_DMA_RESET_VALUE;
      MODULE_DMA.CH[DmaReceiveChannel].DADR.U = SPI_DMA_RESET_VALUE;
    }
    else
    {
      /* Do nothing if QSPI module is not configured */
...
*/
  ...
  if(isDmaTxReqd == 1U)
  {
    ...
    */
    ...
    /*IFX_MISRA_RULE_11_03_STATUS=Type cast to uint32 * is unavoidable*/
    /*IFX_MISRA_RULE_11_05_STATUS="DmaTxSrcPtr" attempt to cast away volatile
      in terms of pointer access.*/
                                             ...
                                             const Spi_DataType* DmaTxSrcPtr)
{
    ...
    */
    if (DataWidth & SPI_32BIT_DATAMASK)
    {
      /*IFX_MISRA_RULE_11_05_STATUS="DmaTxSrcPtr" attempt to cast away
        volatile in terms of pointer access.*/
...
/*******************************************************************************
** Syntax           : IFX_LOCAL_INLINE void Spi_lHwEnableDmaIntr              **
** (                                                                          **
**   uint32 Module                                                            **
** )                                                                          **
**                                                                            **
...
/*******************************************************************************
** Syntax           : IFX_LOCAL_INLINE void Spi_lHwDisableDmaIntr             **
** (                                                                          **
**   uint32 Module                                                            **
** )                                                                          **
**                                                                            **
...
/* #if ((SPI_LEVEL_DELIVERED == 1U) || (SPI_LEVEL_DELIVERED == 2U)) */
                                   ...
                                   || (SPI_LEVEL_DELIVERED == 2U))
/*******************************************************************************
** Syntax           : IFX_LOCAL_INLINE Dma_ErrorStatusType                    **
...
/*******************************************************************************
** Syntax           :  IFX_INLINE void Spi_lHwGpioSetPortPin                  **
** (                                                                          **
**   uint16 CSPin, uint8 CSPolarity                                           **
** )                                                                          **
**                                                                            **
