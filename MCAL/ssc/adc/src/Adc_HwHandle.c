...
/* This value should not have subscript 'U'since its used in Mcal_SetAtomic */
...
#if (ADC_CCU6_SYNCHRONIZATION == STD_ON)
/* Ccu6 initialization */
static void Adc_lCcu6Init(void);
#endif /* (ADC_CCU6_SYNCHRONIZATION == STD_ON) */
...
allowed only for MemMap.h*/
#include "MemMap.h"
/* Debug variable for keren reset status check */
extern uint8 TestAdc_DebugVar;
...
*/
  ...
  /* (ADC_DEV_ERROR_DETECT == STD_ON) || (ADC_SAFETY_ENABLE == STD_ON) */
  {
    ...
    */
    ...
    /* Disable ADC Module */    
    Adc_lWriteClcSfr((uint32)ADC_MODULE_DISABLE);
    ...
    /* This should be done after reset before starting the first conversion to
       reduce analog errors.
    */
    ADC_MODULE_GLOB.GLOBCFG.B.SUCAL = 1U;
    ...
    */
    Adc_lSetResetSre(ADC_ENABLE_INTERRUPT);
    #endif /* (ADC_RESULT_POLLING_MODE == STD_OFF) */
...
Std_ReturnType Adc_InitCheck( const Adc_ConfigType *ConfigPtr )
{
  ...
  if ( Adc_kConfigPtr == ConfigPtr )
  {
    ...
    ReadValue = (uint32)ADC_MODULE_GLOB.CLC.B.DISR;
    CompareValue &= (ReadValue ^ ~((uint32)0));
    
    ReadValue = (uint32)ADC_MODULE_GLOB.CLC.B.EDIS;
    ...
    ReadValue = (uint32)(ADC_MODULE_GLOB.GLOBCFG.U);
    CompareValue &= (ReadValue ^ (~(ConfigValue & ADC_GLOBCFG_MASK)));
    ...
    do
    {
      ...
      ReadValue = (uint32)ADC_MODULE_GLOB.GLOBICLASS[LoopCtr].U;
    ...
    ReadValue = (uint32)ADC_MODULE_GLOB.GLOBEVNP.U;
    CompareValue &= (ReadValue ^ (~(uint32)(ADC_RS2_NODEPOINTER)));

    #if(ADC_USE_EMUX == STD_ON)
  
    /* Checking EMUXSEL register */
    ReadValue = (uint32)ADC_MODULE_GLOB.EMUXSEL.U;
    CompareValue &= (ReadValue ^ ( \
     ...
     )
  {
    ...
    */
                            ...
                            (GrpRequestSrc == ADC_REQSRC3_8STG_QUE) )
    {
      ...
      ADC_MODULE[AdcKernel].VFR.U = (uint32)GrpPtr->IntChMask;
    }
...
)
{
  ...
  /*IFX_MISRA_RULE_17_04_STATUS=Array indexing is used to efficiently 
  access the SFRs of multiple ADC kernels*/
  ReadValue = (uint32)( (ADC_MODULE[AdcKernel].ARBCFG.U) & ADC_ARBCFG_MASK);
  ...
  /*IFX_MISRA_RULE_17_04_STATUS=Array indexing is used to efficiently 
  access the SFRs of multiple ADC kernels*/
  ReadValue = (uint32)ADC_MODULE[AdcKernel].ARBPR.U;
  CompareValue &= (ReadValue ^ (~(uint32)(ConfigValue)));
  ...
  do
  {
    /*IFX_MISRA_RULE_17_04_STATUS=Array indexing is used to efficiently 
    access the SFRs of multiple ADC kernels*/
    ReadValue = (uint32)ADC_MODULE[AdcKernel].ICLASS[LoopCtr].U;
    CompareValue &= (ReadValue ^ ( \
  ...
  ReadValue = (uint32)ADC_MODULE[AdcKernel].SYNCTR.U;
  CompareValue &= (ReadValue ^ (~(uint32)(ConfigValue)));
  ...
  if(AdcKernel == 0U)
  {
    ...
    /*IFX_MISRA_RULE_17_04_STATUS=Array indexing is used to efficiently 
    access the SFRs of multiple ADC kernels*/
    ReadValue = (uint32)ADC_MODULE[AdcKernel].SEVNP.U;
...
IFX_LOCAL_INLINE Std_ReturnType Adc_lSetSreCheck(void)
{
  ...
  do
  {
    if(Adc_lQueueRSConfigured((uint8)LoopCtr) != 0U)
    {
      ...
      ReadValue = (uint32)MODULE_SRC.VADC.G[LoopCtr].SR0.B.SRE;
  ...
  do
  {
    if(Adc_lAutoScanConfigured((uint8)LoopCtr) != 0U)
    {
      ...
      ReadValue = (uint32)MODULE_SRC.VADC.G[LoopCtr].SR1.B.SRE;
      CompareValue &= (ReadValue ^ (~(uint32)(1)));
    }
  ...
  ReadValue = (uint32)MODULE_SRC.VADC.CG[0].SR0.B.SRE;
  CompareValue &= (ReadValue ^ (~(uint32)(1)));
  ...
  ReadValue = (uint32)MODULE_SRC.VADC.CG[0].SR1.B.SRE;
  CompareValue &= (ReadValue ^ (~(uint32)(1)));
  ...
  ReadValue = (uint32)MODULE_SRC.VADC.G[1].SR0.B.SRE;
  CompareValue &= (ReadValue ^ (~(uint32)(1)));
  ...
  do
  {
    if(Adc_lLimitChkConfigured((uint8)LoopCtr) != 0U)
    {
      ...
      ReadValue = (uint32)MODULE_SRC.VADC.G[LoopCtr].SR2.B.SRE;
      CompareValue &= (ReadValue ^ (~(uint32)(1)));
    }
...
static Std_ReturnType Adc_lCcu6InitCheck(void)
{
  ...
  ReadValue = (uint32)MODULE_CCU60.MOSEL.U;
...
{
  #if((ADC_RESULT_HANDLING_MODE == ADC_NON_AUTOSAR) || (ADC_USE_EMUX == STD_ON)\
  ...
  */
  ...
  do
  {
    ...
    /* CHCTR is cleared except Broken Wire Detection bitfields to allow 
    user to configure the BWD. Clearing since CHCTR is not cleared in stop */
    /*IFX_MISRA_RULE_17_04_STATUS=Array indexting is used to efficiently
    access the SFRs of multiple ADC kernels*/
    ADC_MODULE[AdcKernel].CHCTR[IntChNr].U &= \
                                    (unsigned_int)(ADC_CLR_CHCTR_EXCEPT_BWD);
    /* Configure CHCTRx register with Inputclass selction, Reference voltage,
       Result register selction 
    */
    /*IFX_MISRA_RULE_17_04_STATUS=Array indexting is used to efficiently
    access the SFRs of multiple ADC kernels*/
    ADC_MODULE[AdcKernel].CHCTR[IntChNr].U |= (unsigned_int)( ChConfigData |  \
         (uint32)((uint32)ADC_RESULT_ALIGNMENT << (uint32)ADC_RESPOS_BIT_POS) );
    ...
    */
    ...
    #endif /* (ADC_GROUP_EMUX_SCAN == STD_ON) */
    {
      ...
      #endif /* (ADC_ENABLE_LIMIT_CHECK == STD_ON) */
      {
        ...
        /*IFX_MISRA_RULE_17_04_STATUS=Array indexing is used to efficiently
        access the SFRs of multiple ADC kernels*/
        ADC_MODULE[AdcKernel].RCR[IntChNr].U = ADC_RESREG_WFR_ENABLE;
      }
    }
      ...
      )
    {
      ...
      /*IFX_MISRA_RULE_17_04_STATUS=Array indexing is used to efficiently
      access the SFRs of multiple ADC kernels*/
      ADC_MODULE[AdcKernel].RCR[IntChNr].U = ADC_RESULT_ACCUMULATION;
    }
    else
    {
  ...
  /*IFX_MISRA_RULE_17_04_STATUS=Array indexing is used to efficiently
   access the SFRs of multiple ADC kernels*/
  ADC_MODULE[AdcKernel].RCR[IntChNr].B.SRGEN = ADC_ENABLE_RES_INTERRUPT;
...
)
{
    ...
    )
  {
    /*IFX_MISRA_RULE_17_04_STATUS=Array indexing is used to efficiently
    access the SFRs of multiple ADC kernels*/
    ADC_MODULE[AdcKernel].CHCTR[IntChNr].B.SYNC = ADC_SYNC_CHANNEL;
    ...
    do
    {
      ...
      if(KernelPtr != NULL_PTR)
      {
        ...
        if(HwCfgPtr->SyncConvMode != ADC_MASTER)
        {
          ...
          efficiently access the SFRs of multiple ADC kernels*/
          ADC_MODULE[LoopCtr].CHCTR[IntChNr].U |= \
                                                (unsigned_int)( ChConfigData | \
                  ((uint32)ADC_RESULT_ALIGNMENT << (uint32)ADC_RESPOS_BIT_POS));
        }
      }
      LoopCtr++;
  ...
  if(ChCfgPtr[ChIdx].AdcIsChLimitChkEnabled != 0U)
  {
    ...
    */
    ...
    access the SFRs of multiple ADC kernels*/
    ADC_MODULE[AdcKernel].CHCTR[IntChNr].U = 
              (unsigned_int)(ADC_MODULE[AdcKernel].CHCTR[IntChNr].U | SfrValue);
    
    /*IFX_MISRA_RULE_17_04_STATUS=Array indexing is used to efficiently
    access the SFRs of multiple ADC kernels*/
    /*IFX_MISRA_RULE_17_04_STATUS=Array indexing used due to 
    Channel Configuration Structure and is within allowed range*/
    ADC_MODULE[AdcKernel].BOUND.U = ChCfgPtr[ChIdx].AdcLimitChkBnd;
...
)
{
  ...
  switch (GrpReqSrc)
  {
    ...
    case ADC_REQSRC2_BGND_SCAN:
    {
      ...
      ADC_MODULE_GLOB.BRSSEL[AdcKernel].U = ADC_SCAN_EMPTY;
      ...
      */
      ...
      /* When new conversion starts,
         Dummy read for result registerbecomes necessary when 
      ...
      */
      ...
      ADC_MODULE_GLOB.GLOBEFLAG.U = ADC_BACKGRND_RS_EVNT_CLEAR;
      /* Clear the Result register event flag */
      /*IFX_MISRA_RULE_17_04_STATUS=Array indexing is used to efficiently
      access the SFRs of multiple ADC kernels*/
      ADC_MODULE[AdcKernel].REFCLR.U = (uint32)GrpPtr->IntChMask;
      /* Clear the Channel register event flag */
      /*IFX_MISRA_RULE_17_04_STATUS=Array indexing is used to efficiently
      access the SFRs of multiple ADC kernels*/
      ADC_MODULE[AdcKernel].CEFCLR.U = (uint32)GrpPtr->IntChMask;
    }
    ...
    case ADC_REQSRC1_NCH_SCAN:
    {
      ...
      /*IFX_MISRA_RULE_17_04_STATUS=Array indexing is used to efficiently
      access the SFRs of multiple ADC kernels*/
      ADC_MODULE[AdcKernel].ASMR.U = ADC_SCAN_CLR_PENDING_BITS;
      ...
      /*IFX_MISRA_RULE_17_04_STATUS=Array indexing is used to efficiently
      access the SFRs of multiple ADC kernels*/
      ADC_MODULE[AdcKernel].ASCTRL.U = ADC_WRITE_TIMER_MODE_ENABLE;
      ...
      */
      ...
      /*IFX_MISRA_RULE_17_04_STATUS=Array indexing is used to efficiently
      access the SFRs of multiple ADC kernels*/
      ADC_MODULE[AdcKernel].SEFCLR.U = Adc_lClearEventFlag(GrpReqSrc);
      /* Clear the Result register event flag */
      /*IFX_MISRA_RULE_17_04_STATUS=Array indexing is used to efficiently
      access the SFRs of multiple ADC kernels*/
      ADC_MODULE[AdcKernel].REFCLR.U = (uint32)GrpPtr->IntChMask;
    ...
    case ADC_REQSRC3_8STG_QUE:
    {
      ...
      */
      ...
      /*IFX_MISRA_RULE_17_04_STATUS=Array indexing is used to efficiently
      access the SFRs of multiple ADC kernels*/
      ADC_MODULE[AdcKernel].TRCTR.U = (uint32)ADC_CLEAR_TRCTR_REG;
    ...
    /* Queue request source */
    {
      ...
      /*IFX_MISRA_RULE_17_04_STATUS=Array indexing is used to efficiently
      access the SFRs of multiple ADC kernels*/
      ADC_MODULE[AdcKernel].QMR0.U = ADC_QUEUE_MODE_CLEAR;
  ...
  switch (GrpReqSrc)
  {
    ...
    case ADC_REQSRC2_BGND_SCAN:
    {
      ...
      */
      ...
      ADC_MODULE_GLOB.BRSSEL[AdcKernel].U = (uint32)(GrpPtr->IntChMask);

      /* Conversion mode register is programmed for SW trigger */
      ADC_MODULE_GLOB.BRSMR.U = 
               (unsigned_int)(ADC_SCAN_SW_TRIG | ConversionMode | EnableSrcInt);
    ...
    case ADC_REQSRC1_NCH_SCAN:
    {
      ...
      */
      ...
      /*IFX_MISRA_RULE_17_04_STATUS=Array indexing is used to efficiently
      access the SFRs of multiple ADC kernels*/
      ADC_MODULE[AdcKernel].ASSEL.U = (uint32)(GrpPtr->IntChMask);
       
      /* Conversion mode register is programmed for SW trigger */
      /*IFX_MISRA_RULE_17_04_STATUS=Array indexing is used to efficiently
      access the SFRs of multiple ADC kernels*/
      ADC_MODULE[AdcKernel].ASMR.U = 
         (unsigned_int)(ADC_SCAN_SW_TRIG | ConversionMode | EnableSrcInt);
    }
    break; /* Scan Request source */
    #endif /* RS1 used by any of the ADC kernel */
    ...
    #if (ADC_REQSRC0 == ADC_REQSRC_USED )
    {
      ...
      */
      ...
      while (LoopCount < TotalGrpCh)
      {
        ...
        /*IFX_MISRA_RULE_17_04_STATUS=Array indexing is used to efficiently
        access the SFRs of multiple ADC kernels*/
        ADC_MODULE[AdcKernel].QINR0.U = 
                             (unsigned_int)((uint32)IntChNr | ConversionMode);
      ...
      access the SFRs of multiple ADC kernels*/
      ADC_MODULE[AdcKernel].QINR0.U = 
         (unsigned_int)((uint32)IntChNr | ConversionMode | EnableSrcInt);
...
)
{
  ...
  */
  /*IFX_MISRA_RULE_17_04_STATUS=Array indexing is used to efficiently
  access the SFRs of multiple ADC kernels*/
  ADC_MODULE[AdcKernel].SEFCLR.U = Adc_lClearEventFlag(GrpPtr->GrpRequestSrc);
  /* Clear the Result register event flag */
  /*IFX_MISRA_RULE_17_04_STATUS=Array indexing is used to efficiently
  access the SFRs of multiple ADC kernels*/
  ADC_MODULE[AdcKernel].REFCLR.U = (uint32)GrpPtr->IntChMask;
...
)
{
  ...
  switch (GrpReqSrc)
  {
    ...
    case ADC_REQSRC2_BGND_SCAN:
    {
      ...
      {
        /* XTWC = 1, GTWC = 1 */
        ADC_MODULE_GLOB.BRSCTRL.U = 
          (unsigned_int)( (uint32)ADC_WRITE_GAT_TRG_MODE_ENABLE | (TriggSel) | 
                          (TriggMode) | (GateSel)
                        );
        ...
        */
        ADC_MODULE_GLOB.BRSMR.U = 
          (unsigned_int)(ADC_SCAN_EXT_TRIG | (uint32)ADC_CONV_MODE_ONESHOT | 
    ...
    case ADC_REQSRC1_NCH_SCAN:
    {
      ...
      */
      ...
      /*IFX_MISRA_RULE_17_04_STATUS=Array indexing is used to efficiently
      access the SFRs of multiple ADC kernels*/
      ADC_MODULE[AdcKernel].ASCTRL.U = 
        (unsigned_int)( (uint32)ADC_WRITE_TIM_GAT_TRG_MODE_ENABLE | (TriggSel) |
    ...
    #if (ADC_REQSRC0 == ADC_REQSRC_USED )
    {
      ...
      */
      ...
      if (TotalGrpCh != 1U)
      {
        ...
        access the SFRs of multiple ADC kernels*/
        ADC_MODULE[AdcKernel].QINR0.U = 
         (unsigned_int)((uint32)IntChNr | ADC_QUEUE_REFILL |  \
        ...
        */
        ...
        access the SFRs of multiple ADC kernels*/
        ADC_MODULE[AdcKernel].QINR0.U = 
           (unsigned_int)((uint32)IntChNr | ADC_QUEUE_REFILL | EnableSrcInt );
      ...
      /*IFX_MISRA_RULE_17_04_STATUS=Array indexing is used to efficiently
      access the SFRs of multiple ADC kernels*/
      ADC_MODULE[AdcKernel].QMR0.U = 
        (unsigned_int)(ADC_QUEUE_EXT_TRIG | (GateEnable));
    } /* Queue request source */
    #endif /* RS0 used by any of the ADC kernel */
...
)
{
  ...
  */
  ...
  if(TriggSel == ADC_RS3_INTERNAL_TRIG)
  {
    /*IFX_MISRA_RULE_17_04_STATUS=Array indexing is used to efficiently
    access the SFRs of multiple ADC kernels*/
    ADC_MODULE[AdcKernel].TRCTR.U = \
      (unsigned_int)( (GrpPtr->Rs3IntTrigger << (uint32)ADC_ITSEL_BIT_POS) | \
...
void Adc_lGtmTimerCfg(const Adc_GroupCfgType* GrpPtr)
{
  ...
  ((*(Ifx_GTM_TOMx*)(void*)(MODULE_GTM.TOM)).TOM_CH[ModuleNum].           \
                                   CH[ChannelNum].CN0.U) = (unsigned_int)(0U);
  /* Set the Compare register CM0 of TOM */
  /*IFX_MISRA_RULE_11_05_STATUS=volatile typecasting is removed, since it
  gives compile time warning in windriver compiler*/
  ((*(Ifx_GTM_TOMx*)(void*)(MODULE_GTM.TOM)).TOM_CH[ModuleNum]. \
                             CH[ChannelNum].CM0.U) =  (unsigned_int)ReloadVal;
  /* Set the Compare register CM1 of TOM */
  /*IFX_MISRA_RULE_11_05_STATUS=volatile typecasting is removed, since it
  gives compile time warning in windriver compiler*/
  ((*(Ifx_GTM_TOMx*)(void*)(MODULE_GTM.TOM)).TOM_CH[ModuleNum].        \
        CH[ChannelNum].CM1.U) = (unsigned_int)(ReloadVal/ADC_TIMER_VALUE_DIV);
  
  /* Set the Shadow register SR0 of TOM */
  /*IFX_MISRA_RULE_11_05_STATUS=volatile typecasting is removed, since it
  gives compile time warning in windriver compiler*/
  ((*(Ifx_GTM_TOMx*)(void*)(MODULE_GTM.TOM)).TOM_CH[ModuleNum]. \
                              CH[ChannelNum].SR0.U) = (unsigned_int)ReloadVal;
  
  /* Set the Shadow register SR1 of TOM */
  /*IFX_MISRA_RULE_11_05_STATUS=volatile typecasting is removed, since it
  gives compile time warning in windriver compiler*/
  ((*(Ifx_GTM_TOMx*)(void*)(MODULE_GTM.TOM)).TOM_CH[ModuleNum].   \
        CH[ChannelNum].SR1.U) = (unsigned_int)(ReloadVal/ADC_TIMER_VALUE_DIV);
  ...
  if(ChannelNum < ADC_GTM_TOM_TGC0_CHOSEN)
  {
    /*IFX_MISRA_RULE_11_05_STATUS=volatile typecasting is removed, since it
    gives compile time warning in windriver compiler*/
    (*(Ifx_GTM_TOMx*)(void*)(MODULE_GTM.TOM)).TOM_TGC[ModuleNum].
                             TGC[ADC_GTM_TGC0].ENDIS_STAT.U = (unsigned_int) \
  ...
  else
  {
    ...
    gives compile time warning in windriver compiler*/
    (*(Ifx_GTM_TOMx*)(void*)(MODULE_GTM.TOM)).TOM_TGC[ModuleNum].
                                         TGC[ADC_GTM_TGC1].ENDIS_STAT.U =   \
        (unsigned_int)(ADC_GTM_TOM_ENABLE <<  (ADC_GTM_TOM_BITS_PER_CHAN *  \
...
void Adc_lStopGtmTimer(const Adc_GroupCfgType* GrpPtr)
{
  ...
  else
  {
    ...
    (*(Ifx_GTM_TOMx*)(void*)(MODULE_GTM.TOM)).TOM_TGC[ModuleNum].
                                          TGC[ADC_GTM_TGC1].ENDIS_STAT.U =  \
      (unsigned_int)(ADC_GTM_TOM_CHAN_DISABLE << \
   (ADC_GTM_TOM_BITS_PER_CHAN * (ChannelNum - ADC_GTM_TOM_CHANNELS_PER_TGC)));
  }
  ...
  gives compile time warning in windriver compiler*/
  ((*(Ifx_GTM_TOMx*)(void*)(MODULE_GTM.TOM)).TOM_CH[ModuleNum]. \
                                 CH[ChannelNum].CM0.U) =  (unsigned_int)(0U);
  /* Reset the Compare register CM1 of TOM */
  /*IFX_MISRA_RULE_11_05_STATUS=volatile typecasting is removed, since it
  gives compile time warning in windriver compiler*/
  ((*(Ifx_GTM_TOMx*)(void*)(MODULE_GTM.TOM)).TOM_CH[ModuleNum].        \
                                  CH[ChannelNum].CM1.U) = (unsigned_int)(0U);
  
  /* Reset the Shadow register SR0 of TOM */
  /*IFX_MISRA_RULE_11_05_STATUS=volatile typecasting is removed, since it
  gives compile time warning in windriver compiler*/
  ((*(Ifx_GTM_TOMx*)(void*)(MODULE_GTM.TOM)).TOM_CH[ModuleNum]. \
                                  CH[ChannelNum].SR0.U) = (unsigned_int)(0U);
  
  /* Reset the Shadow register SR1 of TOM */
  /*IFX_MISRA_RULE_11_05_STATUS=volatile typecasting is removed, since it
  gives compile time warning in windriver compiler*/
  ((*(Ifx_GTM_TOMx*)(void*)(MODULE_GTM.TOM)).TOM_CH[ModuleNum].   \
                                  CH[ChannelNum].SR1.U) = (unsigned_int)(0U);
...
**                    Disable Hardware                                        **
*******************************************************************************/
void Adc_lResetEruCfg(uint32 HwCfgData)
{
  uint32 EruUnit;
  uint32 EruRegPos;
  uint32 EruReg;
...
**                    Disable Hardware                                        **
*******************************************************************************/
void Adc_lResetEruGateCfg(uint32 HwGateData)
{
  uint32 EruUnit;
  uint32 EruRegPos;
  uint32 EruReg;
...
*******************************************************************************/
void Adc_lSetResetSre(uint32 AdcSreValue)
{
  uint32                  LoopCtr;
  #if ( ADC_GROUP_EMUX_SCAN == STD_ON )
  uint32                  EmuxKernel;
  ...
  do
  {
    ...
    {
      /* ADC SRN0 is used by request source 0 of ADC kernel */
      MODULE_SRC.VADC.G[LoopCtr].SR0.B.SRE = (uint8)AdcSreValue;
    }
  ...
  do
  {
    ...
    {
      /* ADC SRN1 is used by request source 1 of ADC kernel */
      MODULE_SRC.VADC.G[LoopCtr].SR1.B.SRE = (uint8)AdcSreValue;
    }
  ...
  MODULE_SRC.VADC.CG[0].SR0.B.SRE = (uint8)AdcSreValue;
  ...
  MODULE_SRC.VADC.CG[0].SR1.B.SRE = (uint8)AdcSreValue;
  ...
  MODULE_SRC.VADC.G[1].SR0.B.SRE = (uint8)AdcSreValue;
  ...
  do
  {
    if(Adc_lLimitChkConfigured((uint8)LoopCtr) != 0U)
    {
      ...
      MODULE_SRC.VADC.G[LoopCtr].SR2.B.SRE = (uint8)AdcSreValue;
    }
  ...
  do
  {
    if(Adc_lKernelConfigured((uint8)LoopCtr) != 0U)
    {
      ...
      MODULE_SRC.VADC.G[LoopCtr].SR3.B.SRE = (uint8)AdcSreValue;
    }
  ...
  do
  {
    ...
    /* ADC SRN3 is used for Non AS Dma transfer of ADC kernel */
    MODULE_SRC.VADC.G[AdcKernel].SR3.B.SRE = (uint8)AdcSreValue;
...
)
{
  ...
  do
  {
    ...
    /*IFX_MISRA_RULE_17_04_STATUS=Array indexing is used to efficiently
    access the SFRs of multiple ADC kernels*/
    ChInpClass=(uint32)(ADC_MODULE[AdcKernel].CHCTR[ChannelNum].B.ICLSEL); 
    ...
    if( (ChInpClass & ADC_GLOBAL_INPUT_CLASS) )
    {
      ...
      ChResol = (uint32)(ADC_MODULE_GLOB.GLOBICLASS[ChInpClass].B.CMS);
    }
    else
    {
      ...
      /*IFX_MISRA_RULE_17_04_STATUS=Array indexing is used to efficiently
      access the SFRs of multiple ADC kernels*/
      ChResol = (uint32)(ADC_MODULE[AdcKernel].ICLASS[ChInpClass].B.CMS);
    }
...
static void Adc_lCcu6Init(void)
{
  ...
  if(Adc_kConfigPtr->GlobCfgPtr->Ccu6Kernel == 1U)
  {
    ...
    Mcal_ResetENDINIT(); 
    Ccu6BaseRegPtr->CLC.U = 0U;
    Mcal_SetENDINIT();
    /* Read back the CLC register */
    ReadBack = Ccu6BaseRegPtr->CLC.U;
  }
  ...
  Ccu6RegPtr->T13PR.U = (unsigned_int)Adc_kConfigPtr->GlobCfgPtr->PeriodValue;
  /* configures duty cycle */
  Ccu6RegPtr->CC63SR.U = (unsigned_int)Adc_kConfigPtr->GlobCfgPtr->DutyValue;
  /* bit ECT130 = 1 serves to route  CC63ST signals out to  COUT63 */
  Ccu6RegPtr->MODCTR.U = ADC_CCU6_MODCTR_VAL;
  /* set bit T13STR  & T13RS  -> enable shadow transfer & start timer T13 */
  Ccu6RegPtr->TCTR0.U = (unsigned_int)Adc_kConfigPtr->GlobCfgPtr->T13Clock;
  /* set bit T13STR  & T13RS  -> enable shadow transfer & start timer T13 */
  Ccu6RegPtr->TCTR4.U = ADC_CCU6_TCTR4_VAL;
  /* CCU60_COUT63 routed to output signal CCU6061 TRIG1 */
  MODULE_CCU60.MOSEL.U = Adc_kConfigPtr->GlobCfgPtr->TrigSel;

} /* Adc_lCcu6Init */

#if (ADC_DEINIT_API == STD_ON)
/*******************************************************************************
** Syntax           : void Adc_lCcu6DeInit                                    **
**                    (                                                       **
**                       void                                                 **
**                    )                                                       **
**                                                                            **
...
static void Adc_lResetSfr(void)
{
  ...
  Adc_lWriteKernelRstSfr( ADC_KERNEL_RESET );
  ...
  do
  {
     WaitCount--;
     #ifdef IFX_ADC_DEBUG
     RstStatus =(uint32)(ADC_MODULE_GLOB.KRST0.B.RSTSTAT & TestAdc_DebugVar);
     #else
     RstStatus =(uint32)(ADC_MODULE_GLOB.KRST0.B.RSTSTAT);
     #endif
  }while((RstStatus == 0U)&&(WaitCount > 0U));
  
  /* Clear Reset Status */
  Mcal_ResetENDINIT();
  ADC_MODULE_GLOB.KRSTCLR.B.CLR = 1U;
  Mcal_SetENDINIT();
