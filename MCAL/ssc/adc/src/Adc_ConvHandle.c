...
)
{
  ...
  if (GrpPtr->TriggSrc == ADC_TRIGG_SRC_HW)
  {
    ...
    if (EruGateFlag == (uint32)ADC_GATE_ERU_SELECTED)
    {
      ...
      Adc_lResetEruGateCfg(HwGateData);
    }
    #endif /* (ADC_ERU_GATING== STD_ON) */
...
)
{
  ...
  /* Set Source Address */ 
  Mcal_DmaSetSourceAddr(DmaChannel, SrcAddress);
  /* Set Destination Address */ 
  Mcal_DmaSetDestAddr(DmaChannel, Address);
  /* Clear channel control register */
  Mcal_DmaChClrCtlCfg(DmaChannel); 
  /* Clear interrupt on DMA Channel */
  Mcal_DmaChClrIntrFlags(DmaChannel);
  /* Set Interrupt Control and Interrupt Pointer */
  Mcal_DmaCfgIntControl(DmaChannel, ADC_DMA_CH_INTCT);
  /* Set DMA Channel control register */
  /* After the configured number of transfer, DMA interrupt is generated */
  Mcal_DmaSetDataWidth(DmaChannel, ADC_DMA_CH_WIDTH);
  Mcal_DmaSetTxCount(DmaChannel, DataPtr->DmaData.DmaUpdateCount);
  /* Enable DMA Interrupt */
  Mcal_DmaEnableChIntrTrigger(DmaChannel);
  Mcal_DmaEnableIntr(DmaChannel);
  /* DMA Address control */
  Mcal_DmaSetAdicr(DmaChannel, ADC_DMA_ADR_CTR);
  /* Enable ADC HW trigg on selected DMA Channel */
  Mcal_DmaEnableHwTransfer(DmaChannel);
...
IFX_LOCAL_INLINE void Adc_lDmaChDeInit(const Dma_ChannelType DmaChannel)
{
  ...
  Mcal_DmaDisableHwTransfer(DmaChannel);
  
  /* Reset DMA Channel control register */
  Mcal_DmaChClrCtlCfg(DmaChannel);

  /* ReSet Interrupt Control and Interrupt Pointer */
  Mcal_DmaChClrIntctEtrlFlags(DmaChannel);

  /* Disable DMA Interrupt */
  Mcal_DmaDisableIntr(DmaChannel);
  /* DMA Address control */
  Mcal_DmaClrAdcr(DmaChannel);
